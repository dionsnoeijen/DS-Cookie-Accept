...
class Ds_cookie_accept 
{
	...
	public function css() {
		...
		$background_color_fallback = $this->EE->TMPL->fetch_param('background_color_fallback', '#000000');
		$background_color = $this->EE->TMPL->fetch_param('background_color', '0, 0, 0, 0.8');

		if(!$accepted_cookies) {
			$css = '<style type="text/css">';
			$css 	.= 'div#cookiemessage {position:fixed;bottom:0;text-align:center;width:100%;padding:5px 0;background:'. $background_color_fallback .';background:rgba('.$background_color.');z-index:999;}';
			$css 	.= 'div#cookiemessage a,div#cookiemessage p {font-family: Arial, Helvetica, sans-serif; color:#ffffff;margin:0;padding:0; font-size:13px;}';
			...
			$css 	.= 'div#ds_cookie_accept_privacy_details {width:100%; max-width:960px; text-align:left;display:none;padding-bottom:30px;margin:0 auto;}';
